...
The basic switch fabric contain just two (2) audio paths, so this limit how many function that can happen at one time. For example if extension 1 goes offhook it gets dialtone, if extension 2 goes offhook after that at the same time, there will be no dialtone and a dead line. This because of the limited switch fabric and audio paths. Telephones connected to the SLIC can talk to another phone connect to a SLIC via IN/OUT audio pins. The In from one SLIC goes to the Out and vice versa, this cross connection is provided with audio path B in the basic switch fabric.
